...
if(where=="local"){
  ...
  path <- "/vast/palmer/home.mccleary/jc3893/"
}
setwd(path)
...
season <- "breeding"; months <- 6:7; mindate=152; maxdate=212; scode=2
...
# define scale
scale <- 10 # options are 1, 3, 5, 10, 50
plotres <- 100 # corresponding 500, 300, 200, 100, 50

# ebird data as ff object
if (season=="resident"){
  for (s in c("breeding","nonbreeding",
              "prebreeding_migration","postbreeding_migration")){
    ebird_ff <- read.csv.ffdf(
      file=paste0("/gpfs/gibbs/pi/jetz/data/temporary_datasets/scale/ebird_",s,"_CEA.csv"),
  ...
  ebird_full <- read.csv.ffdf(
    file=paste0("/gpfs/gibbs/pi/jetz/data/temporary_datasets/scale/ebird_",season, "_CEA.csv"),
    colClasses=c(rep("factor", 2), rep("numeric",2),
                 rep("integer",2), "numeric", "integer",
                 "numeric","integer",rep("integer",677),
                 rep("numeric",264)))
...
# prediction surface as ff object
pred_surface_full <- read.csv.ffdf(
  file=paste0("/gpfs/gibbs/pi/jetz/data/temporary_datasets/scale/habitat_prediction-surface_",scale,".csv"),
...
bbox <- readOGR("30x30/shapefiles", "30x30BBox_NAmerica") %>%
  spTransform(countries, CRS="+proj=cea +lat_ts=30 +lon_0=0 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs ") %>%
  st_as_sf()
grid <- raster(paste0("30x30/grids/Global_reference_raster_",scale,"km_CEA.tif")) %>%
  crop(bbox)
...
# species range backups (Map of life)
      ...
      if(!file.exists(paste0(sci_name,"_complete_",season,"_",scale,".csv"))){
        ...
        setwd(dir)
          ...
          writeOGR(sprange, paste0(sci_name,"/rangemap"),
                   ...
                   overwrite_layer=T)
          try(file.remove(paste0(path,code,"-range-mr-2020.gpkg")))
          ...
          # Check if there's enough prediction surface
          ...
          for (k in 1:5){
            ...
            # random split out-of-bag (OOB) samples from train data
          ...
          write_csv(train[,c("y","x","pres_abs")], 
            ...
            # Test Set predictive performance metrics
            ...
            {#Select Optimal Threshold on train OOB data
              ...
              write_csv(pa.metrics, 
                        paste0(sci_name,"/accuracy/accuracy_matrix_",
                               sci_name,"_",season,"_",scale,"_00.csv"))
              }
            }
            {# Compute Test set PPMs
              ...
              assign(paste0("pa.metrics.",k), pa.metrics)
            }
          }
          
          # Averaging across folds
          pa.metrics = colMeans(rbind(pa.metrics.1[2:6], pa.metrics.2[2:6], 
                                     pa.metrics.3[2:6], pa.metrics.4[2:6], 
                                     pa.metrics.5[2:6]))
            ...
            pred_surface$occ_thresh <- ifelse(
              pred_surface$occ_pred>pa.metrics[1],1,0)
            ...
            writeRaster(ras, paste0(sci_name,"/predictions/ROR/PO_predictions_",
                                    sci_name,"_",season,"_",scale,"_00.tif"),
                        overwrite=T)
            ...
            {# Plot maps
              jpeg(paste0("plots/",scale,"/PO_spatial_predictions_",
                          sci_name,"_",season,"_",scale,"_00.jpeg"),
            ...
            modName <- paste0(sci_name,"_",season,"_",scale)
